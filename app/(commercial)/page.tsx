...
export default function Home() {
  ...
  const handleBuyerClick = () => router.push('/fr/acheteur')
  const handleEditorClick = () => router.push('/fr/editeur')
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
        <div className="text-center">
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900 mb-2">Plateforme Side By SaaS</h1>
          ...
          {process.env.NEXT_PUBLIC_MARKETING_URL && (
            ...
            >
              Découvrir Side By SaaS (site commercial) →
            </a>
          )}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col md:flex-row items-center justify-between">
            ...
            <div className="flex items-center space-x-6 text-sm text-slate-400">
              <button onClick={() => router.push('/fr/docs')} className="hover:text-white transition-colors">Documentation</button>
              <button onClick={() => router.push('/fr/about')} className="hover:text-white transition-colors">À propos</button>
              <button onClick={() => router.push('/fr/contact')} className="hover:text-white transition-colors">Contact</button>
            </div>
          </div>
        </div>
      </footer>
